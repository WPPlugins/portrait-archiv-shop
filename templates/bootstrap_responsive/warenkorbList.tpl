		...
		foreach ($position->images as $image) {
			if ($image->count > 0) {
				?>
					<tr>
						<td>
							<a href="<?php echo add_query_arg (array('pDetails'=> $image->imageId, 'pawps_showWarenkorb' => null)); ?>" title="Bild anzeigen"><img src='<?php echo $image->getImage()->getThumbUrl(); ?>' border='0' class='img-thumbnail' /></a>
						</td>
